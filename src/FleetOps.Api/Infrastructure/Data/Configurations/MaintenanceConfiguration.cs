...
public class MaintenanceConfiguration : IEntityTypeConfiguration<Maintenance>
{
    public void Configure(EntityTypeBuilder<Maintenance> builder)
    {
        ...
        _ = builder.Property(x => x.Status).HasDefaultValue(MaintenanceStatus.Planned).IsRequired();
        _ = builder.HasIndex(x => new { x.VehicleId, x.PlannedDate });
        _ = builder.HasIndex(x => x.MaintenanceTypeId);
        _ = builder.HasOne(x => x.Vehicle).WithMany(x => x.Maintenances).HasForeignKey(x => x.VehicleId).OnDelete(DeleteBehavior.NoAction);
        _ = builder.HasOne(x => x.MaintenanceType).WithMany(x => x.Maintenances).HasForeignKey(x => x.MaintenanceTypeId).OnDelete(DeleteBehavior.NoAction);
    }
}
